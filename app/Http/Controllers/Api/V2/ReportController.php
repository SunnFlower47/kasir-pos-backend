...
class ReportController extends Controller
{
    ...
    public function purchases(Request $request): JsonResponse
    {
        ...
        $applyPurchaseFilters = function ($builder, bool $prefixed = false) use ($request, $showAllData, $dateFrom, $dateTo, $effectiveStatus) {
            ...
            $tenantColumn = $prefixed ? 'purchases.tenant_id' : 'tenant_id';

            $builder->where($statusColumn, $effectiveStatus);
            $builder->where($tenantColumn, Auth::user()->tenant_id);
        ...
        if ($request->get('summary') === 'true') {
            ...
            $itemsBreakdownQuery = DB::table('purchase_items')
                ...
                ->where('purchases.tenant_id', Auth::user()->tenant_id) // Secure explicit DB query
                ->selectRaw('
                    products.name,
                    products.sku,
        ...
        for ($date = $startDate->copy(); $date->lte($endDate); $date->addDay()) {
            $dayQuery = DB::table('purchases')
                ->where('tenant_id', Auth::user()->tenant_id) // Secure explicit DB query
    ...
    public function stocks(Request $request): JsonResponse
    {
        ...
        if ($request->has('date_from') && $request->has('date_to')) {
            ...
            $query = DB::table('product_stocks')
                ...
                ->where('products.tenant_id', Auth::user()->tenant_id) // Use product's tenant_id as trusted scope
                ->select(
            ...
            $topStockProducts = DB::table('stock_movements')
                ...
                ->join('categories', 'products.category_id', '=', 'categories.id')
                ->where('products.tenant_id', Auth::user()->tenant_id) // Secure explicitly
                ->selectRaw('
                    products.id,
                    products.name,
                    products.sku,
            ...
            for ($date = $startDate->copy(); $date->lte($endDate); $date->addDay()) {
                $dayData = DB::table('stock_movements')
                    ->join('products', 'stock_movements.product_id', '=', 'products.id') // Join needed to check tenant
                    ->where('products.tenant_id', Auth::user()->tenant_id) // Secure
                    ->whereDate('stock_movements.created_at', $date->format('Y-m-d'))
                    ->selectRaw('
