...
class TransactionController extends Controller
{
    ...
    public function store(StoreTransactionRequest $request): JsonResponse
    {
        ...
        try {
            ...
            // OPTIMIZATION: Batch Load Products, Stocks, and Units to prevent N+1
            $productIds = collect($request->items)->pluck('product_id')->unique();
            $unitIds = collect($request->items)->pluck('unit_id')->filter()->unique();

            // 1. Load Products
            $products = Product::whereIn('id', $productIds)->get()->keyBy('id');

            // 2. Load Stocks (Locked)
            // Locks all relevant rows to prevent race conditions during bulk update
            ...
            // 3. Load Unit Conversions
            $productUnits = \App\Models\ProductUnit::whereIn('product_id', $productIds)
                        ->whereIn('unit_id', $unitIds)
                        ->get(); // Can't easily keyBy pair, will filter in loop

            // Create transaction items and reduce stock
            foreach ($request->items as $item) {
                // Get pre-loaded product
                ...
                // Get pre-loaded stock
                $productStock = $stocks->get($product->id);
                ...
                // Robustly determine conversion factor from Pre-loaded Collection
                $conversionFactor = 1;
                $productUnit = null;
                
                if ($unitId != $product->unit_id) {
                    // Find matching unit in pre-loaded collection
                    $productUnit = $productUnits->first(function($pu) use ($product, $unitId) {
                        return $pu->product_id == $product->id && $pu->unit_id == $unitId;
                    });
                ...
                // Reduce stock (Atomic update on the loaded model instance)
                // Note: reduceStock likely performs a DB update. 
                // Since $productStock is active record, this works.
