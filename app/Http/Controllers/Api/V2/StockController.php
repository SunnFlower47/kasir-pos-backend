...
class StockController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // Ensure we only retrieve stocks for products that exist and are visible (tenant-scoped)
        $query->has('product');
        ...
        $stocks->getCollection()->transform(function ($stock) {
            // Safety check in case product loaded as null (though has('product') should prevent this)
            $stock->is_low_stock = $stock->product ? $stock->quantity <= $stock->product->min_stock : false;
            return $stock;
        });
    ...
    public function transfer(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // OPTIMIZATION: Batch Load & Lock Stocks
            $productIds = collect($request->items)->pluck('product_id');
            
            // 1. Lock Source Stocks
            $sourceStocks = ProductStock::whereIn('product_id', $productIds)
                                     ->where('outlet_id', $request->from_outlet_id)
                                     ->lockForUpdate()
                                     ->get()
                                     ->keyBy('product_id');

            // 2. Lock Destination Stocks
            $destStocks = ProductStock::whereIn('product_id', $productIds)
            ...
            foreach ($request->items as $item) {
                // Get Source from Batch
                $sourceStock = $sourceStocks->get($item['product_id']);
                ...
                // Get Dest from Batch
                $destStock = $destStocks->get($item['product_id']);

                // If not found in batch, try to find freshwater or create (Fallback)
                // This handles the edge case where stock record doesn't exist yet
                if (!$destStock) {
                    // Try to find one more time (maybe created by race condition? Unlikely due to lock)
                    // Or just create it.
                ...
                // Use standardized stock update methods
                // reduceStock/addStock usually perform direct DB update or increment/decrement on the model
