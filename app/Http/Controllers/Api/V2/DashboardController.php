...
class DashboardController extends Controller
{
    ...
    public function outletComparison(Request $request): JsonResponse
    {
        ...
        $outlets = Outlet::where('is_active', true)
            ...
            ->withCount(['transactions as transactions_count' => function ($q) use ($rangeStart, $rangeEnd) {
                $q->where('status', 'completed');
                if ($rangeEnd) {
                    $q->whereBetween('transaction_date', [$rangeStart, $rangeEnd]);
                } else {
                    $q->where('transaction_date', '>=', $rangeStart);
                }
            }])
            ->withSum(['transactions as revenue' => function ($q) use ($rangeStart, $rangeEnd) {
                $q->where('status', 'completed');
                if ($rangeEnd) {
                    $q->whereBetween('transaction_date', [$rangeStart, $rangeEnd]);
                } else {
        ...
        foreach ($outlets as $outlet) {
            // Calculate Low Stock independently or optimize later. 
            // Since low stock calculation involves joins and logic, doing it per outlet is acceptable if N is small (<50).
            // But we can optimize it with a single aggregation query if needed. 
            // For now, eliminating Transaction queries is the biggest win (Transactions >>> Outlets).
            
            $lowStockCount = $outlet->productStocks()
                    ->join('products', 'product_stocks.product_id', '=', 'products.id')
                    ->where('product_stocks.quantity', '>', 0) 
                    ->whereRaw('product_stocks.quantity <= products.min_stock')
                    ->count();
            ...
            $comparison[] = [
                'outlet' => $outlet,
                'transactions_count' => $outlet->transactions_count ?? 0,
                'revenue' => (float) ($outlet->revenue ?? 0),
                'avg_transaction_value' => $outlet->transactions_count > 0 
                    ? ($outlet->revenue / $outlet->transactions_count) 
                    : 0,
                'active_users' => $outlet->users_count ?? 0,
