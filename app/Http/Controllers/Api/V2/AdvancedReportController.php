...
class AdvancedReportController extends Controller
{
    ...
    private function getCustomerAnalytics(string $dateFrom, string $dateTo, ?int $outletId): array
    {
        ...
        $newCustomers = $baseQuery->whereNotNull('customer_id')
            ->whereRaw('customer_id NOT IN (SELECT DISTINCT customer_id FROM transactions WHERE transaction_date < ? AND customer_id IS NOT NULL AND tenant_id = ?)',
                [$dateFrom . ' 00:00:00', Auth::user()->tenant_id])
            ->distinct()
            ->count('customer_id');

        $returningCustomers = $baseQuery->whereNotNull('customer_id')
            ->whereRaw('customer_id IN (SELECT DISTINCT customer_id FROM transactions WHERE transaction_date < ? AND customer_id IS NOT NULL AND tenant_id = ?)',
                [$dateFrom . ' 00:00:00', Auth::user()->tenant_id])
            ->distinct()
            ->count('customer_id');
        ...
        $allProducts = Product::leftJoin('product_stocks', function($join) use ($outletId) {
            ...
            ->selectRaw("
                ...
                COALESCE((
                    ...
                    AND t.tenant_id = '" . Auth::user()->tenant_id . "'
